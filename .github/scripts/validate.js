...
data.courses.forEach(function(c,i){
  ["id","name","day","startPeriod","endPeriod","weeks"].forEach(function(k){ if(c[k]===undefined) fail("course["+i+"] 缺字段 "+k); });
...
var p=data.semester.startDate.split("-").map(Number);
var start=new Date(p[0],p[1]-1,p[2]); var now=new Date();
var week=Math.floor(Math.round((now-start)/86400000)/7)+1;
